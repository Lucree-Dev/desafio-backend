...
def custom_error(message, status_code) -> object:
    ...
    That method is indicated to use with error during the validation. The method has the same
...
def sucessfully(message, status_code):
    """
    This method receive a message and status code and convert it json using the flask methods.
    That method is indicated to use with sucess during the validation. The method has the same
